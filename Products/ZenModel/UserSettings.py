...
class UserSettingsManager(ZenModelRM):
    ...
    def exportXmlHook(self, ofile, ignorerels):
        map(
            lambda x: x.exportXml(ofile, ignorerels),
...
class UserSettings(ZenModelRM):
    ...
    security.declareProtected(ZEN_CHANGE_ADMIN_OBJECTS, 'manage_editAdministrativeRoles')
    @validate_csrf_token
    def manage_editAdministrativeRoles(self, ids=(), role=(), REQUEST=None):
        ...
        for ar in self.adminRoles():
            ...
            try:
                i = ids.index(mobj.managedObjectName())
            except ValueError:
                continue
            mobj = mobj.primaryAq()
    ...
    security.declareProtected(ZEN_CHANGE_ADMIN_OBJECTS, 'manage_deleteAdministrativeRole')
    @validate_csrf_token
    def manage_deleteAdministrativeRole(self, delids=(), REQUEST=None):
        ...
        for ar in self.adminRoles():
            mobj = ar.managedObject()
            if mobj.managedObjectName() not in delids:
                continue
            mobj = mobj.primaryAq()
